...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler,
        IEndDragHandler, IPointerDownHandler, IPointerUpHandler, IPointerEnterHandler, 
        IPointerExitHandler
    {
        #region Fields
        ...
        // Events
        [SerializeField] private UnityEvent<GameObject, float> onTransitionEffects =
            new UnityEvent<GameObject, float>();
        ...
        [SerializeField]
        private UnityEvent<int, int> onPanelCentering = new UnityEvent<int, int>();

        [SerializeField]
        private UnityEvent<int, int> onPanelCentered = new UnityEvent<int, int>();
        ...
        private bool ValidConfig
        {
            get
            {
                ...
                {
                    Debug.LogError(
                        "<b>[SimpleScrollSnap]</b> Snapping speed cannot be negative.",
                        gameObject);
                    valid = false;
                }
        ...
        void SetupCursors()
        {
            ...
            if (defaultCursor == null || handOpen == null || handClosed == null)
            {
                handIconsLoaded = false;
            }
            else
            {
                handIconsLoaded = true;
            }
        ...
        public void AssignPanels(RectTransform[] panels)
        {
            Panels = panels;
        ...
        private void Setup()
        {
            ...
            // Scroll-Rect
            ScrollRect.horizontal = (movementType == MovementType.Free) ||
                                    (movementAxis == MovementAxis.Horizontal);
            ScrollRect.vertical = (movementType == MovementType.Free) ||
            ...
            for (int i = 0; i < NumberOfPanels; i++)
            {
                ...
                {
                    Panels[i].anchorMin =
                        new Vector2(movementAxis == MovementAxis.Horizontal ? 0f : 0.5f,
                    ...
                    float x =
                        (automaticLayoutMargins.Right + automaticLayoutMargins.Left) /
                        2f - automaticLayoutMargins.Left;
                    float y =
                        (automaticLayoutMargins.Top + automaticLayoutMargins.Bottom) /
                        2f - automaticLayoutMargins.Bottom;
            ...
            if (movementType == MovementType.Fixed)
            {
                ...
                if (useAutomaticLayout)
                {
                    ...
                    float contentWidth = (movementAxis == MovementAxis.Horizontal)
                        ? (NumberOfPanels * (automaticLayoutSpacing + 1f) * size.x) -
                          (size.x * automaticLayoutSpacing)
                        : size.x;
                    float contentHeight = (movementAxis == MovementAxis.Vertical)
                        ? (NumberOfPanels * (automaticLayoutSpacing + 1f) * size.y) -
            ...
            if (isSelected)
            {
                if (!((isDragging || isPressing) && useSwipeGestures))
                {
                    SnapToPanel();
                     ...
                     (ScrollRect.velocity.magnitude <= thresholdSpeedToSnap ||
                      thresholdSpeedToSnap == -1f))
            {
                SelectPanel();
            }
        }
            ...
            if (useOcclusionCulling && (Velocity.magnitude > 0f || forceUpdate))
            {
                for (int i = 0; i < NumberOfPanels; i++)
                {
                    switch (movementAxis)
                    {
                        case MovementAxis.Horizontal:
                            Panels[i].gameObject
                                .SetActive(Mathf.Abs(GetDisplacementFromCenter(i).x) <=
                                           Viewport.rect.width / 2f + size.x);
                            break;
                        case MovementAxis.Vertical:
                            Panels[i].gameObject
                                .SetActive(Mathf.Abs(GetDisplacementFromCenter(i).y) <=
                ...
                switch (movementAxis)
                {
                    case MovementAxis.Horizontal:
                        for (int i = 0; i < NumberOfPanels; i++)
                        {
                            ...
                            else if (GetDisplacementFromCenter(i).x <
                                     Content.rect.width / -2f)
                            {
                                Panels[i].anchoredPosition += offset;
                            }
                        }
                    ...
                    case MovementAxis.Vertical:
                        for (int i = 0; i < NumberOfPanels; i++)
                        {
                            ...
                            else if (GetDisplacementFromCenter(i).y <
                                     Content.rect.height / -2f)
                            {
                                Panels[i].anchoredPosition += offset;
                            }
                        }
        ...
        private void SnapToPanel()
        {
            ...
            {
                if (GetDisplacementFromCenter(CenteredPanel).magnitude <
                    (Viewport.rect.width / 10f))
                {
        ...
        public void Add(GameObject panel, int index)
        {
            ...
            {
                Debug.LogError(
                    "<b>[SimpleScrollSnap]</b> Index must be an integer from 0 to " +
                    NumberOfPanels + ".", gameObject);
                return;
            }
        ...
        public void Remove(int index)
        {
            if (NumberOfPanels == 0)
            {
                Debug.LogError("<b>[SimpleScrollSnap]</b> There are no panels to remove.",
                    gameObject);
                return;
            }
        ...
        {
            return Panels[index].anchoredPosition + Content.anchoredPosition -
